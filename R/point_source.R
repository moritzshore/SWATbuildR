#' Add point sources (recall) to the SWAT+ model setup.
#'
#' The point source locations are derived from a point shape file input layer.
#' Optionally point source records are added if csv files with the same name as
#' the name of a point source exist in the same folder with the point source
#' location layer
#'
#' @param point_path Path of the point source point shape layer
#' @param data_path Path of the project data folder
#' @param max_point_dist Maximum distance to snap a point source to the closest
#'   channel or reservoir object. Numeric value in meters.
#'
#' @returns Writes the following tables into tables.sqlite:
#'  - recall.recall_con
#'  - recall.recall_con_out
#'  - recall.recall_dat
#'  - recall.recall_rec
#'
#' @importFrom DBI dbConnect dbDisconnect dbReadTable dbWriteTable
#' @importFrom dplyr arrange bind_rows filter group_by mutate select ungroup %>%
#' @importFrom RSQLite SQLite
#' @importFrom sf read_sf st_crs st_drop_geometry st_transform
...
#' Find the minimum distance of a point to another vector object.
#'
#' The function returns a tibble with the point id and the object id and
#' distance to the nearest feature of a second vector input layer.
#'
#' @param pnt Point source vector layer as sf object
#' @param obj Object input layer as sf object for which the closest feature to a
#'   point is identified.
#' @param typ Character label that defines the type of object.
#'
#'
#' @returns A tibble with the closest distances of a point source to a water
#'   object.
#'
...
#' Prepare point source time series data.
#'
...
#' was provided in the data folder.
#'
#' The default data is a constant input with all zeros for all variables.
#'
#' @param tbl Tibble of user input point source data.
