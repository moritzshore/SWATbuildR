...
calc_wd <- function(contr_area) {
  # US equations from https://doi.org/10.1111/jawr.12282
...
#' @importFrom sf read_sf st_area st_drop_geometry
#' @importFrom terra rast mask minmax zonal
#' @importFrom tibble tibble
#' @importFrom whitebox wbt_isobasins wbt_reclass_from_file
#'
#' @keywords internal
#'
calculate_contributing_area <- function(data_path, cha_rst, dem_res) {
  ...
  ca_stat <- map(seq(0.4, 1, 0.1), ~ zonal(iso_acc_cha, cha_rst, fun = quantile,
                                           probs = .x, na.rm = T)) %>%
