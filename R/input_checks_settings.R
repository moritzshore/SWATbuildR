...
#' @param method Only used for raster. Method used for estimation of new cell
#'   values when projecting. E.g. use 'near' for soil classes and 'bilinear'
#'   (default) for elevation.
#'
#' @returns The transformed input layer as sf object
#'
...
#' @export
#'
check_project_crs <- function(layer, data_path, proj_layer, label, type, method = 'bilinear') {
  ...
  if(layer_wkt != proj_wkt) {
    if (proj_layer) {
      if(type == 'raster') {
        layer <- project(layer, proj_wkt, method = method)
      } else {
...
#' @param data_path Path of the project data folder
#'
#' @returns Writes the table 'project.wkt' into the temporary data base
#' '/tables.sqlite'.
#'
...
#' Check attribute columns of read input layers.
#'
...
#'
#' @returns The checked layer as sf object with renamed columns.
#'
#' @importFrom dplyr any_of mutate rename select %>%
#'
#' @export
#'
...
#'
#' The function triggers an error if the area fraction
...
#'
#' @returns The functions returns an error if the coverage of a polygon is < cov_frc.
#'
#' @importFrom dplyr  %>%
#' @importFrom sf read_sf
#' @importFrom terra classify vect zonal
#'
#' @export
#'
...
#' Check the IDs of draining land objects to channel objects.
#'
