#' Prepare terrain properties for the land objects.
#'
...
#'
#' @returns Writes hydrologically corrected flow pointer and flow accumulation
#'   raster layers into data/raster.
#'
...
#' Calculate the land object connectivity.
#'
#' The routine uses the prepared flow accumulation and the flow pointer that
#' were derived from the DEM with the surface channels and the reservoirs burnt
#' in and iterates over all land object IDs to calculate the net flux fractions
#' between the land objects and the water objects.
#'
#' @param data_path Path of the project data folder
#'
...
#'
#' @returns A table with the from and to object IDs with the corresponding
#'   flow accumulations
#'
...
#' Identify the coordinates to where current position points.
#'
...
#'
#' @returns the raster value from the matrix sub based on the pointer value
#'   pnt_i.
#'
#' @keywords internal
#'
...
#'
#' @returns vector of length 2 with row/column indices.
#'
#' @keywords internal
#'
...
#'
#' Remove connections with connectivity fractions lower than frc_thres.
#' This is necessary, because keeping all connections between objects
#' increases the risk of circuit routing (water is routed in circles without
#' ever ending up in channel or reservoir).
#'
#' @param data_path Path of the project data folder
...
#'
#' @returns The table with the land object connections where connections with
#'   fractions lower than `frc_thres` were removed.
#'
#' @importFrom DBI dbConnect dbDisconnect dbReadTable dbWriteTable
#' @importFrom dplyr arrange filter group_by mutate select ungroup %>%
#' @importFrom RSQLite SQLite
#'
#' @export
#'
